cptca <- function(gene.name,
  ...
  # -----------------------
  # 2. 读取 cancer-PDC info.csv 并获取 PDC 编码
  # -----------------------
  pdc_info_file <- file.path(base_path, "cancer-PDC info.csv")
  if (!file.exists(pdc_info_file)) {
    stop("Error: 'cancer-PDC info.csv' file not found. Please check the path or filename.")
  }
  ...
  if (length(matched_rows) == 0) {
    stop("Error: The input 'cancer.type' was not found in 'cancer-PDC info.csv'.")
  }

  # 函数：根据组学类型返回对应的 PDC 列名
  get_pdc_col_name <- function(omics) {
    switch(omics,
           "Transcriptome"    = "PDC_RNA",
           "Proteome"         = "PDC_Pro",
           "Phosphoproteome"  = "PDC_Phospro",
           stop("Unknown omics type: ", omics))
  }
