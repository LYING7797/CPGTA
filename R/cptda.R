cptda <- function(cancer.type,
  ...
  info_file <- file.path(base_path, "cancer-PDC info.csv")
  if (!file.exists(info_file)) {
    stop("Error: 'cancer-PDC info.csv' does not exist in the specified path.")
  }
  ...
  res_df$group <- "none"
  res_df$group[res_df$log2FC < -threshold] <- "down"
  res_df$group[res_df$log2FC > threshold] <- "up"
